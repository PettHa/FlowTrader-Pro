// frontend/src/components/builder/nodes/ExitNode.jsx
import React, { useState, useCallback, useEffect } from 'react';
import { Handle, Position } from 'reactflow';
import { LogOut, ChevronDown, ChevronUp, ArrowUp, ArrowDown } from 'lucide-react';

const ExitNode = ({ data, selected, id, isConnectable, updateNodeData }) => {
  ...
  const handleBlur = useCallback((fieldName, value) => {
    ...
    //   console.log(`ExitNode ${id} blur update:`, { [fieldName]: processedValue });
  ...
  const getHeaderTitle = () => {
    ...
    return `${isLong ? 'Long' : 'Short'} Exit`;
  };

  return (
    <div className={`strategy-node action-node exit-node ${selected ? 'selected' : ''} ${isLong ? 'long' : 'short'} ${isExpanded ? 'expanded' : 'collapsed'}`}>
      <div className="node-header" onClick={toggleExpand} title={isExpanded ? 'Collapse' : 'Expand'}>
        ...
        {getPositionIcon()}
        <LogOut size={14} className="node-header-icon" />
        ...
        <>
          ...
          <div className="node-param-group">
            ...
            <input id={`${id}-label`} type="text" name="label" className="node-input"
              ...
              placeholder={`${isLong ? 'Long' : 'Short'} Exit`}
            />
          </div>
        </>
      )}
...
export default React.memo(ExitNode);